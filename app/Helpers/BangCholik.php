...
if (!function_exists('isBotRunning')) {
    function isBotRunning(): bool
    {
        $output = shell_exec('wmic process where "CommandLine like \'%bot.js%--from-laravel%\' and Name=\'node.exe\'" get ProcessId');
        return preg_match('/\d{3,}/', $output) === 1;
    }
}

if (!function_exists('killBot')) {
    function killBot(): void
    {
        shell_exec('wmic process where "CommandLine like \'%bot.js%--from-laravel%\' and Name=\'node.exe\'" call terminate');
    }
}
...
if(!function_exists('BCL_renderSelectIcon')){
    function BCL_renderSelectIcon($icon)
    {
        ...
        foreach ( $icons as $i){
            ...
            $html .= '<option data-content="<i class=\''.$i.'\' aria-hidden=\'true\'></i>"  value="'.$i.'" '.$selected.'> '.$i.'</option>';
        }
        $html .= '</select>';
        return $html;
}
...
if(!function_exists('BCL_menuTree')){
    function BCL_menuTree($parent_id = 0)
    {
        $items = '';
        $query=Menu::where('parent_id',$parent_id);
        if($query->count()>0){
            $result=$query->orderBy('urut')
                ->get();
            $items .= '<ol class="dd-list">';
            foreach ($result as $row) {
                $items .= BCL_renderMenuItem($row->id, $row->label, $row->icon, $row->url);
                $items .= BCL_menuTree($row->id);
                $items .= '</li>';
            }
            $items .= '</ol>';
        }
...
if(!function_exists('BCL_menuSidebar')){
    function BCL_menuSidebar($event)
    {
...
if(!function_exists('BCL_ArrToString')){
    function BCL_ArrToString($data)
    {
        $str="";
        $d=0;
