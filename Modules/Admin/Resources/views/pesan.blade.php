          ...
          <div class="card-body d-flex justify-content-between align-items-center">
            <div>
              <h5 class="card-title text-dark">
                <strong>Dari : {{ $item->nama_pelapor }}</strong>
                <small class="text-muted" data-tanggal="{{ \Carbon\Carbon::parse($item->updated_at ?? $item->created_at)->format('Y-m-d') }}">{{ \Carbon\Carbon::parse($item->updated_at ?? $item->created_at)->format('H:i:s || d-m-Y') }}</small>
              </h5>
                ...
                <table class="table table-striped">
                    <thead>
                        <tr class="text-center">
                            ...
                            <th class="col-md-1 text-center">Diteruskan</th>
                            <th class="col-md-1 text-center">Dibalas</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr class="text-center">
                            ...
                            <td class="col-md-1 text-center">
                                {{ \Carbon\Carbon::parse($item->pending)->format('H:i:s') }}<br>
                                {{ \Carbon\Carbon::parse($item->pending)->format('d-m-Y') }}
                            </td>
                            <td class="col-md-1 text-center">
                                {{ $item->processed_at ? \Carbon\Carbon::parse($item->processed_at)->format('H:i:s') : '-' }}<br>
                                {{ $item->processed_at ? \Carbon\Carbon::parse($item->processed_at)->format('d-m-Y') : '' }}
                            </td>
                            <td class="col-md-1 text-center">
                                {{ $item->completed_at ? \Carbon\Carbon::parse($item->completed_at)->format('H:i:s') : '-' }}<br>
        ...
        <div class="modal-body">
          <div class="mb-3">
            ...
            <select id="filterUnit" class="form-select">
              ...
              @foreach ($pesan->pluck('unit_name')->unique() as $unit)
...
@push('js')
    ...
    <script>
        ...
        document.getElementById('filterStatus').addEventListener('change', applyLiveFilter);
        ...
        function applyLiveFilter() {
            ...
            items.forEach((item) => {
                ...
                const statusText = item.dataset.status; 
                ...
                if (matchUnit && matchStatus && matchKeyword && matchDate) {
