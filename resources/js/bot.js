...
client.on('message', async msg => {
    ...
    if (messageText.toLowerCase().startsWith("jawaban dari saya adalah")) {
        ...
        return; // Hentikan proses setelah menangani jawaban
    }
            ...
            async (err, results) => {
                ...
                db.query(
                    ...
                    async (err, results) => {
                        ...
                        const now = new Date();
                        let fileNama = null; // Inisialisasi filePath sebagai null
                        ...
                        if (msg.hasMedia) {
                            try {
                                ...
                                // Ambil ekstensi dari mimetype, misal image/jpeg → jpeg, image/png → png
                                const mimeType = media.mimetype.split('/')[1];  // 'jpeg' atau 'png'
                                ...
                                const fileName = `${Date.now()}_${msg.id.id}.jpg`; // bisa sesuaikan ekstensi
                                fileNama = fileName; // Simpan nama file untuk disimpan di DB
                                filePath = path.join(__dirname, '../../public/media', fileName);
    ...
    // =============================
        ...
        if (messageText.toLowerCase().includes("nomor pelapor :") &&
            ...
            // Memastikan pemisahan dengan memeriksa setiap baris dan memastikan tidak ada kesalahan format
            lines.forEach(line => {
                const lower = line.trim().toLowerCase();  // Gunakan trim() untuk menghapus spasi ekstra
